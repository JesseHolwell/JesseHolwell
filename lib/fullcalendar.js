        ...
        $.each(input,
            function(name, val) {
                if (name != "views") {
                    ...
                    if (
                        $.isPlainObject(val) &&
                            !/(time|duration|interval)$/i
                            .test(name) && // exclude duration options. might be given as objects
                            $
                            .inArray(name, complexOptions) ==
                            -1 // complex options aren't allowed to be View-Option-Hashes
                    ) {
                        subObj = null;

                        // iterate through the properties of this possible View-Option-Hash value
                        $.each(val,
                            function(subName, subVal) {
...
// reduces the available height.
    function distributeHeight(els, availableHeight, shouldRedistribute) {
        ...
        var minOffset2 = Math
            .floor(availableHeight - minOffset1 * (els.length - 1)); // for last element *FLOORING NOTE*
    ...
    var GridCoordMap = Class.extend({
        ...
        computeBounds: function() {
            this.bounds = this.containerEl
                ? getClientRect(this.containerEl)
                : // area within scrollbars
                null;
        },
    ...
    var CellDragListener = DragListener.extend({
        ...
        dragStart: function(ev) {
            ...
            cell = this.getCell(ev
                .pageX,
                ev.pageY); // might be different from this.origCell if the min-distance is large
                ...
                function(ev) {
                    if (
                        !$(ev.target)
                            ...
                            !$(ev.target).closest(".fc-popover")
                            .length // not on a popover (like the "more.." events one)
                    ) {
                ...
                .on("click",
                    function(ev) {
                        ...
                        if (typeof clickOption === "function") {
                            ...
                            clickOption = view.trigger("eventLimitClick",
                                null,
                                {
                                    date: date,
                                    dayEl: dayEl,
    ...
    var TimeGrid = Grid.extend({
        ...
        slatRowHtml: function() {
            ...
            while (slotTime < this.maxTime) {
                slotDate = this.start.clone()
                    .time(slotTime); // after .time() will be in UTC. but that's good, avoids DST issues
        ...
        renderFill: function(type, segs, className) {
            ...
            if (segs.length) {
                ...
                segs = this
                    .renderFillSegEls(type, segs); // assignes `.el` to each seg. returns successfully rendered segs
    ...
    var View = fc.View = Class.extend({
        ...
        computeScrollerHeight: function(totalHeight) {
            ...
            both.css({
                ...
                left: -1
// ensure reflow in case the el was already relative. negative is less likely to cause new scroll
            });
            ...
            if (currentView) {
                ...
                if (
                    !currentView.displaying ||
                        !date.isWithin(currentView
                            .intervalStart,
                            currentView.intervalEnd) // implicit date window change
                ) {
                    if (elementVisible()) {
...
// options that should be computed off live calendar options (considers override options)
    ...
    function populateInstanceComputableOptions(options) {
        $.each(instanceComputableOptions,
            function(name, func) {
...
// text explicitly set by calendar's constructor options. overcomes icons
                                ...
                                if (buttonName == "title") {
                                    ...
                                    if ((customButtonProps = (calendar.options.customButtons || {})[buttonName])) {
                                        ...
                                        defaultText = options
                                            .buttonText[buttonName]; // everything else is considered default
                                    }
        ...
        function buildEventSource(sourceInput) { // will return undefined if invalid source
            ...
            if (source) {
                ...
                if ($.isArray(source.events)) {
                    ...
                    source.events = $.map(source.events,
                        function(eventInput) {
                ...
                if (abstractEvent._recurring) {
                    ...
                    while (date.isBefore(_rangeEnd)) {
                        ...
                        if (!dowHash || dowHash[date.day()]) { // if everyday, or this particular day-of-week
                            ...
                            startTime = abstractEvent
                                .start; // the stored start and end properties are times (Durations)
        ...
        for (i = 0; i < cache.length; i++) {
            ...
            if (
                !event ||
                    event
                    ._id !==
                    otherEvent._id // don't compare the event to itself or other related [repeating] events
            ) {
...
// It is responsible for managing width/height.
    ...
    var BasicView = View.extend({
        ...
        renderDates: function() {
            ...
            this.dayGrid.coordMap.containerEl = this
                .scrollerEl; // constrain clicks/etc to the dimensions of the scroller
    ...
    var MonthView = BasicView.extend({
        ...
        computeRange: function(date) {
            ...
            if (this.isFixedWeeks()) {
                rowCnt = Math.ceil(range.end.diff(range
                    .start,
                    "weeks",
                    true)); // could be partial weeks due to hiddenDays
