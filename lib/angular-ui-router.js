    ...
    UrlMatcher.prototype.format = function(values) {
        ...
        function encodeDashes(str) { // Replace dashes with encoded "\-"
            return encodeURIComponent(str).replace(/-/g,
                function(c) { return "%5C%" + c.charCodeAt(0).toString(16).toUpperCase(); });
        }
    ...
    function $UrlMatcherFactory() {
        ...
        this.Param = function Param(id, type, config, location) {
            ...
            function unwrapShorthand(config) {
                ...
                var isShorthand = indexOf(keys, "value") === -1 &&
                    indexOf(keys, "type") === -1 &&
            ...
            function getReplace(config, arrayMode, isOptional, squash) {
                ...
                return filter(defaultPolicy, function(item) { return indexOf(configuredKeys, item.from) === -1; })
                    .concat(replace);
            }
    ...
    function $StateProvider($urlRouterProvider, $urlMatcherFactory) {
        ...
        var stateBuilder = {
            ...
            views: function(state) {
                ...
                forEach(isDefined(state.views) ? state.views : { '': state },
                    function(view, name) {
        ...
        this.$get = $get;
        $get.$inject = [
            "$rootScope", "$q", "$view", "$injector", "$resolve", "$stateParams", "$urlRouter", "$location",
            "$urlMatcherFactory"
        ];
        ...
        function $get($rootScope,
            ...
            $state.reload = function reload(state) {
                return $state.transitionTo($state.current,
                    $stateParams,
                    { reload: state || true, inherit: false, notify: true });
            };
            ...
            $state.transitionTo = function transitionTo(to, toParams, options) {
                ...
                if (shouldSkipReload(to, toParams, from, fromParams, locals, options)) {
                    ...
                    if (options.location && to.navigable && to.navigable.url) {
                        $urlRouter.push(to.navigable.url,
                            toParams,
                            {
                                $$avoidResync: true,
                ...
                if (options.notify) {
                     ...
                     * </pre>
                     */
                    if ($rootScope.$broadcast("$stateChangeStart", to.self, toParams, from.self, fromParams, options)
                        .defaultPrevented) {
                ...
                var transition = $state.transition = resolved.then(function() {
                    ...
                    function(error) {
                         ...
                         */
                        evt = $rootScope.$broadcast("$stateChangeError",
                            to.self,
                            toParams,
                            from.self,
                            fromParams,
            ...
            $state.get = function(stateOrName, context) {
                if (arguments.length === 0)
                    return map(objectKeys(states), function(name) { return states[name].self; });
                ...
                function resolveViews() {
                    ...
                    forEach(state.views,
                        function(view, name) {
                            ...
                            viewsPromises.push($resolve.resolve(injectables, dst.globals, dst.resolve, state)
                                .then(function(result) {
                                    // References to the controller (only instantiated at link time)
                                    if (isFunction(view.controllerProvider) || isArray(view.controllerProvider)) {
                                        var injectLocals = angular.extend({}, injectables, dst.globals);
                                        result.$$controller = $injector
                                            .invoke(view.controllerProvider, null, injectLocals);
                                    } else {
        ...
        function shouldSkipReload(to, toParams, from, fromParams, locals, options) {
            ...
            if (!options.reload &&
                to === from &&
                (locals === from.locals ||
                    (to.self.reloadOnSearch === false && nonSearchParamsEqual(from, fromParams, toParams)))) {
                return true;
            }
        }
    }
 ...
 */
    function $ViewScrollProvider() {
         ...
         */
        this.$get = [
            "$anchorScroll", "$timeout", function($anchorScroll, $timeout) {
                if (useAnchorScroll) {
                    return $anchorScroll;
                }
                            ...
                            function(clone) {
                                renderer.enter(clone,
                                    ...
                                    function onUiViewEnter() {
                                        ...
                                        if (angular.isDefined(autoScrollExp) && !autoScrollExp ||
    ...
    function $StateRefDirective($state, $timeout) {
        return {
            ...
            link: function(scope, element, attrs, uiSrefActive) {
                ...
                def.options = extend(defaultOpts(element, $state),
