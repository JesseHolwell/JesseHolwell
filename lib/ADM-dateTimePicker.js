...
(function(angular) {
    ...
    var ADMdtpConvertor = function() {
        ...
        function getYearJalaliCalender(daysPassedInJalaliCalender) {
            ...
            if (years > 22) {
                ...
                if (daysPassedInJalaliCalender - (sal * 365) - leapsCount - (isLeapYearInJalaliCalender(sal) ? 1 : 0) >
                    0) {
                    sal++;
                    return sal;
    ...
    var ADMdtpFactory = function(ADMdtpConvertor) {
        ...
        this.parseString = function(str, format) {
            var _keys = [], _date = {};
            var formats = [
                "YY/MM/DD", "YY/MM/DD hh:mm", "YY-MM-DD", "YY-MM-DD hh:mm", "MM/DD/YY", "MM-DD-YY", "MM/DD/YY hh:mm",
                "MM-DD-YY hh:mm"
            ];
            formats.unshift(format);

            for (var i = 0, j = format.length; i < j; i++) {
                var _isValid = new RegExp(formats[i].replace(/[a-z]+/gi,
                    function(key) {
                ...
                for (var i = 0, j = arr.length; i < j; i++) {
                    if (typeof arr[i] == "number") {
                        ...
                        if (arr[i].indexOf("d") == -1 && arr[i].indexOf("i") == -1) {
                            ...
                            var _inMonthValid = new
                                RegExp("^[!]?(([0-9]?[0-9])?[d]([+][0-9][0-9]?)?)([&]([0-9]?[0-9])?[d]([+][0-9][0-9]?)?)*?$").test(arr[i]);
                            var _inWeekhValid = new RegExp("^[!]?([i]([+][0-9][0-9]?)?)([&][i]([+][0-9][0-9]?)?)*?$")
                                .test(arr[i]);
                            ...
                            if (_inMonthValid || _inWeekhValid) {
                                ...
                                if (_inMonthValid) {
                                    ...
                                    _patt.forEach(function(item) {
                                        ...
                                        for (var k = 0; k < 31; k++) {
                                            if (_params[0] != 1 && k % _params[0] == _params[1] ||
                                                _params[0] == 1 && k == _params[1])
                                                _tmpObj[k] = 1;
                                        }
                                    });
                                    for (var k = 0; k < 31; k++) {
        ...
        return {
            ...
            link: function(scope, element, attrs, admDtp) {
                ...
                scope.selectThisDay = function(day) {
                    ...
                    //Auto Change to time picker
                    scope.timePickerStat = true;
                ...
                scope.changeTimeValue = function(variable, value) {
                    var _num = (Number(scope.time[variable]) + value + ((variable == "hour") ? 24 : 60)) %
                        ((variable == "hour") ? 24 : 60);
                    ...
                    if (scope.dtpValue) {
                        if (scope.minDate || scope.maxDate) {
                            ...
                            if ((scope.minDate && !ADMdtpFactory.isDateBigger(_dateTime, scope.minDate)) ||
                                (scope.maxDate && !ADMdtpFactory.isDateBigger(scope.maxDate, _dateTime)))
                                return;
                        }
                    }
                ...
                scope.modelChanged = function(input) {
                    ...
                    if (!_inputUnix ||
                    ((scope.minDate && !ADMdtpFactory.isDateBigger(_inputUnix, scope.minDate)) ||
                        (scope.maxDate && !ADMdtpFactory.isDateBigger(scope.maxDate, _inputUnix)))) {
        ...
        return {
            ...
            link: function(scope, element, attrs, ngModel) {
                ...
                scope.openCalendar = function() {
                    ...
                    var _admDtpCalendarHtml = angular
                        .element('<adm-dtp-calendar style="opacity:0;"></adm-dtp-calendar>');
                ...
                function($scope) {
                    ...
                    this.fillDays = function(date, noTransition) {
                        ...
                        if ($scope.calType == "jalali") {
                            ...
                            var _pDate = ADMdtpConvertor
                                .toJalali(_ite_date.getFullYear(), _ite_date.getMonth() + 1, _ite_date.getDate());
                        ...
                        while (true) {
                            ...
                            if (_disable && _thisDay < 8)
                                if (($scope.calType == "jalali" && _ite_date.getDay() == 6) ||
                                    ($scope.calType == "gregorian" && _ite_date.getDay() == 0))
                                    break;
    ...
    function upTo(el, className) {
        ...
        while (el && el.parentElement) {
            el = el.parentElement;
            if (el.className && el.className.toLowerCase().includes(className)) {
                return el;
            }
        }
        ...
        return {
            ...
            link: function($scope, elem, attr) {
                var classList = (attr
                        .outsideIfNot !==
                        undefined)
                    ? attr.outsideIfNot.replace(", ", ",").split(",")
                    : [];
                ...
                $document.on("click contextmenu",
                    function(e) {
                            ...
                            if (id !== undefined) {
                                for (i = 0; i < classList.length; i++) {
                                    if (id.indexOf(classList[i]) > -1 ||
                                        (typeof classNames == "string" && classNames.indexOf(classList[i]) > -1)) {
                                        return;
                                    }
                                }
                            }
                        ...
                        if (e.target.className == "ADMdtp-container")
                            return;

                        var parentElement = upTo(e.target, "ADMdtp-container");
    ...
    return angular.module("ADM-dateTimePicker", [])
        ...
        .directive("admDtp",
            ["ADMdtp", "ADMdtpConvertor", "ADMdtpFactory", "constants", "$compile", "$timeout", ADMdtpDirective])
        .directive("admDtpCalendar",
            ["ADMdtp", "ADMdtpConvertor", "ADMdtpFactory", "constants", "$timeout", ADMdtpCalendarDirective])
